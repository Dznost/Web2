const fs = require("fs") // Sử dụng require cho Node.js modules
const path = require("path") // Sử dụng require cho Node.js modules
...
exports.deleteBook = async (req, res) => {
    try {
        ...
        // Kiểm tra xem có yêu cầu mượn sách đang chờ xử lý hoặc đã mượn cho cuốn sách này không
        ...
        if (activeBorrowRequests > 0) {
            return res.status(400).json({ error: "Không thể xóa sách vì có yêu cầu mượn đang chờ xử lý hoặc đã mượn." })
        }

        // Xóa ảnh bìa nếu không phải là ảnh mặc định
        ...
        await book.deleteOne() // Sử dụng deleteOne() thay vì remove()

        // Tùy chọn: Xóa các yêu cầu sách và yêu cầu mượn liên quan (nếu không phải là activeBorrowRequests)
        await BookRequest.deleteMany({ bookId: book._id }) // Nếu bạn có trường bookId trong BookRequest
        await BorrowRequest.deleteMany({ bookId: book._id, status: { $nin: ["borrowed", "pending", "approved"] } }) // Xóa các yêu cầu không còn active
    ...
    try {
        ...
        if (!book) {
            return res.status(404).render("error", {
                ...
                message: "Sách không tồn tại để chỉnh sửa.",
            })
        }
...
exports.updateBook = async (req, res) => {
    try {
        ...
        // Xử lý ảnh bìa
        if (req.file) {
            // Nếu có ảnh mới, xóa ảnh cũ (nếu không phải mặc định)
            if (book.image && book.image !== "/images/default-book.jpg") {
            ...
            // Nếu người dùng chọn ảnh mặc định và trước đó có ảnh khác
            if (book.image && book.image !== "/images/default-book.jpg") {
                const oldImagePath = path.join(__dirname, "..", "public", book.image)
                fs.unlink(oldImagePath, (err) => {
                    if (err) console.error("Lỗi khi xóa ảnh cũ:", err)
        ...
        // Nếu không có file mới và không chọn default, giữ nguyên ảnh cũ (existingImage sẽ là path cũ)
