...
exports.confirmBorrow = async (req, res) => {
	try {
		...
		await Notification.create({
			userId: user._id,
			message: `Bạn đã mượn sách \"${borrowRequest.bookId.title}\" thành công. Ngày trả: ${borrowRequest.returnDate.toLocaleDateString()}`,
		})
...
exports.confirmReturn = async (req, res) => {
	try {
		...
		if (!borrowRequest.borrowDate) return res.status(400).json({ success: false, error: "Không có ngày mượn." })
		const actualDate = new Date(actualReturnDate)
		if (isNaN(actualDate.getTime()) || actualDate <= borrowRequest.borrowDate) {
